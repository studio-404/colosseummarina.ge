<?php defined('DIR') OR exit; ?>
		<div id="title" class="fix">
			...
			<div class="name"><?php echo ($action=='add') ? a('addsetting') : a('editsetting').': '.$setting["title"];?></div>
		</div>	
...
<?php $ulink = ($action=="add") ? ahref($section, 'add') : ahref($section, 'edit', array('key'=>$setting["key"])); ?>
	                <form id="catform" method="post" action="<?php echo $ulink;?>">
                   		<input type="hidden" name="settings_form_perform" value="1" />
<?php if($action=="add") { ?>
                        <div class="list2 fix">
                            <div class="name"><?php echo a('title');?> <span class="star">*</span></div>					
                            <input type="text" id="title" name="title" value="<?php echo ($action=="edit") ? $setting["title"] : '' ;?>" class="inp"/>
                        </div>	
                        <div class="list fix">
                            <div class="name"><?php echo a('name');?> <span class="star">*</span></div>					
                            <input type="text" id="key" name="key" value="<?php echo ($action=="edit") ? $setting["key"] : '' ;?>" class="inp"/>
                        </div>	
<?php } else {?>
                            <input type="hidden" id="key" name="key" value="<?php echo ($action=="edit") ? $setting["key"] : '' ;?>" class="inp"/>
<?php } ?>
                        <div class="list2 fix">
                            <div class="name"><?php echo a('value');?> <span class="star">*</span></div>					
                            <input type="text" id="value" name="value" value="<?php echo ($action=="edit") ? $setting["value"] : '' ;?>" class="inp"/>
                        </div>	
					</form>
				</div>	
