...
//    $undeletable = c('section.undeletable');
    ...
    foreach ($results AS $result):
        ...
        $is_first = ($count == 1);
        $is_last = ($count >= $total);
        $arrow_up = $is_first ? '<img src="_cadmin/img/none.png" width="9" height="9" style="float:left;" />' : '<a href="' . ahref($action, 'move', array('where' => 'up', 'idx' => $result['idx'], 'menu' => $result['menuid'])) . '"><img src="_cadmin/img/buttons/icon_arrow_up_' . ($result['level'] > 4 ? 1 : $result["level"] - 1) . '.png" class="star" title="' .  a('tt.moveup') . '" width="9" height="9" /></a>';
        $arrow_down = $is_last ? '<img src="_cadmin/img/none.png" width="9" height="9" style="float:left;" />' : '<a href="' . ahref($action, 'move', array('where' => 'down', 'idx' => $result['idx'], 'menu' => $result['menuid'])) . '"><img src="_cadmin/img/buttons/icon_arrow_down_' . ($result['level'] > 4 ? 1 : $result["level"] - 1) . '.png" class="star" title="' .  a('tt.movedn') . '" width="9" height="9" /></a>';
		...
		$pad .= $lnks . '<img src="_cadmin/img/' . $collapse . '.png" width="16" height="16" style="margin:0 0 0 ' . ((($result['level'] - 1) * 25) + 5) . 'px;" >' . $lnke;		
		$pad .= '<img src="_cadmin/img/buttons/' . $bn . '.png" style="margin:0 10px 0 0;" >';
		$spad .= '<img src="_cadmin/img/none.png" style="margin:0 5px 0 ' . ((($result['level'] - 1) * 25) + 5) . 'px;" >&nbsp;';
		$ch = ($result["visibility"]=='true') ? '' : 'un';
?>
					
                    <div class="<?php echo $class;?> fix" id="div<?php echo $result["id"];?>" >
						<div class="check">
							<a href="javascript:chclick(<?php echo $result['idx'];?>);"><img src="_cadmin/img/buttons/icon_<?php echo $ch;?>visible.png" class="star" title="<?php echo a('tt.visibility');?>" id="img_<?php echo $result['idx'];?>" style="width:9px;height:9px;" /></a>
...
<?php echo mb_substr(c('site.url') . l() . '/' . $result["slug"], 0,84,'UTF-8') . ((mb_strlen(c('site.url') . l() . '/' . $result["slug"],'UTF-8')>84) ? '...' : '');?>
                        ...
                        <div class="action fix">
                        	<a href="<?php echo href($result['id']) ?>" target="_blank"><img src="_cadmin/img/buttons/icon_preview.png" class="star" title="<?php echo a('ql.preview');?>" /></a>
                            <a href="<?php echo ahref($action, 'edit', array('menu'=>$result['menuid'], 'id'=>$result["id"], 'idx'=>$result["idx"]));?>"><img src="_cadmin/img/buttons/icon_edit.png" class="star" title="<?php echo a('ql.edit');?>" /></a>
<?php
...
?>
							<a href="<?php echo ahref($menulink1,'show', array('menu'=>$attachedid)) ?>"><img src="_cadmin/img/buttons/icon_files<?php echo ($page_count['cnt']==0)	? '' : '_r';?>.png" class="star" title="<?php echo a('ql.editcontent');?>" /></a>
<?php
	} else {
?>
							<a href="javascript:"><img src="_cadmin/img/buttons/icons_no_files.png" class="star" title="" style="width:15px;height:15px;" /></a>
<?php } ?>     
<?php                       
	$att_count = db_fetch("SELECT count(*) AS cnt FROM " . c("table.attached") . " WHERE `page` = {$result['idx']};");		
?>
                            <a href="<?php echo ahref('files', 'show', array('menu'=>$result['menuid'], 'id'=>$result["id"], 'idx'=>$result["idx"]));?>"><img src="_cadmin/img/buttons/icon_attach<?php echo ($att_count['cnt']==0)	? '' : '_r';?>.png" class="star" title="<?php echo a('ql.attachments');?>" /></a>
                            <a href="javascript:addr('Page Address:', '<?php echo href($result['id']) ?>');"><img src="_cadmin/img/buttons/icon_link.png" class="star" title="<?php echo a('ql.link');?>" /></a>
<?php if($action =='sitemap') : ?>
							<a href="<?php echo ahref($action, 'add', array('menu'=>$result['menuid'], 'id'=>$result["id"], 'idx'=>$result["idx"], 'level'=>$result["level"]));?>"><img src="_cadmin/img/buttons/icon_add.png" class="star" title="<?php echo a('ql.addsubpage');?>" /></a>
<?php endif; ?>
<?php
	$undeletable = 0;
	...
	if($undeletable == 0) {
?>
                            <a href="javascript:del(<?php echo $result['id'];?>, '<?php echo $result['title'];?>');"><img src="_cadmin/img/buttons/icon_delete.png" class="star" title="<?php echo a('ql.delete');?>" /></a>
<?php } else { ?>
							<a href="javascript:"><img src="_cadmin/img/buttons/icon_delete_inactive.png" class="star" title="" style="width:15px;height:15px;" /></a>
<?php } ?>     
                        </div>
...
?>
					<ul id="page" class="fix left" style="width:900px;">
						<li><a href="<?php echo ahref($action, $subaction, $params) . '&start=' . $first;?>"><img src="_cadmin/img/prev2.png" width="5" height="5" alt=""  class="arrow" /></a></li>
						<li><a href="<?php echo ahref($action, $subaction, $params) . '&start=' . $prev;?>"><img src="_cadmin/img/prev.png" width="5" height="5" alt=""  class="arrow" /></a></li>
<?php
...
<?php } ?>
						<li><a href="<?php echo ahref($action, $subaction, $params) . '&start=' . $next;?>"><img src="_cadmin/img/next.png" width="5" height="5" alt="" class="arrow" /></a></li>
						<li><a href="<?php echo ahref($action, $subaction, $params) . '&start=' . $last;?>"><img src="_cadmin/img/next2.png" width="5" height="5" alt="" class="arrow" /></a></li>
					</ul>
<?php } ?>
<?php if($action=='sitemap') { ?>
