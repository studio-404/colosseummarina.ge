...
?>
		...
		<div id="box">
   			...
   			<form id="dataform" name="dataform" method="post" action="<?php echo ahref('userrights', 'save', array());?>">
			<div id="part">
				<div id="top" class="fix">
                    <a href="javascript:selectUser();" class="button br" style="float: right;"><?php echo a("selectuser");?></a>
                    <div style="float:right;padding-top:4px;">
                    	&nbsp;<select name="userid" id="userid" class="iinp-small">
...
?>                
				...
				<div id="user">
					...
					<div class="list2 fix">
						<div style="float:left; font-weight:bold; font-size:11px; white-space:nowrap;"><input type="checkbox" class="inp" name="allprivileges" style="margin-top:2px;" id="allprivileges" onclick="javascript:allow();" />&nbsp;<?php echo a('allprivileges');?></div>
						<div style="margin-left:10px; float:left; font-weight:bold; font-size:11px; white-space:nowrap;"><input type="checkbox" class="inp" name="noprivileges" style="margin-top:2px;" id="noprivileges" onclick="javascript:deny();" />&nbsp;<?php echo a('noprivileges');?>&nbsp;</div>
					</div>
...
<?php
	...
	foreach($user_access as $access) :
		switch($access["action"]) :
			...
			case "gallerylist" : 	$acc_gallerylist = 1; break;
...
?>
					<div class="list fix">
						...
						<input type="checkbox" style="float:left; padding-right:80px" name="files" <?php echo (($selected_user!='')&&($acc_filemanager==1)) ? 'checked="checked"' : '';?> />						<div class="users-name" style="margin-left:15px;"><?php echo a("filemanager");?> </div>					
					...
					<div class="list2 fix">
						<input type="checkbox" style="float:left; padding-right:80px" name="polls" <?php echo (($selected_user!='')&&($acc_polls==1)) ? 'checked="checked"' : '';?> />
						<div class="users-name" style="margin-left:15px;"><?php echo a("polls"); ?> </div>					
						
						<input type="checkbox" style="float:left; padding-right:80px" name="catalogs" <?php echo (($selected_user!='')&&($acc_catalogs==1)) ? 'checked="checked"' : '';?> />
						<div class="users-name" style="margin-left:15px;"><?php echo a("catalogs"); ?> </div>					
						
					</div>

					<div class="list fix">
						<input type="checkbox" style="float:left; padding-right:80px" name="faqs" <?php echo (($selected_user!='')&&($acc_faqs==1)) ? 'checked="checked"' : '';?> />
						<div class="users-name" style="margin-left:15px;"><?php echo a("faq"); ?> </div>					
						
						<input type="checkbox" style="float:left; padding-right:80px" name="banners" <?php echo (($selected_user!='')&&($acc_bannergroups==1)) ? 'checked="checked"' : '';?> />
						<div class="users-name" style="margin-left:15px;"><?php echo a("banners"); ?> </div>					
				...
				<div id="bottom" class="fix">
                    ...
                    <a href="javascript:selectUserAlt();" class="button br" style="float: right;"><?php echo a("selectuser");?></a>
                    <div style="float:right;padding-top:5px;">
                    	&nbsp;<select name="userid1" id="userid1" class="inp-small">
                        	<option value="0" style="color:#cbcbcb" ><?php echo a("selectuser");?></option>
<?php
	$users = db_fetch_all("SELECT * FROM `" . c('table.users') . "` WHERE `deleted` = 0 AND `class` = 1 AND `usercat` = 'User';");
	foreach($users as $user) :
...
function selectUser() {
	userid = $("#userid").val();
	window.location="<?php echo ahref('userrights', 'show') . '&userid=';?>" + userid;
}

function selectUserAlt() {
	userid = $("#userid1").val();
	window.location="<?php echo ahref('userrights', 'show') . '&userid=';?>" + userid;
}
